...
Production bug로 볼 만한 명확한 API contract 불일치는 확인하지 못했다. 감사 당시 response timestamp/date-time 형식은 API Spec 예시가 `+09:00` offset을 기준으로 쓰인 반면 실제 DTO와 테스트는 `Instant`의 UTC `Z` 출력을 계약처럼 고정하고 있었다. `Decide API date-time response format` follow-up에서 persisted response timestamp와 `ErrorResponse.timestamp` 모두 UTC `Z` 형식을 공식 API contract로 결정했다. `Clarify PassProduct optional fields in API Spec` follow-up에서 PassProduct `price`와 `validDays`의 optional request 및 nullable response 동작도 API Spec에 명시했다. AdminUser activate/deactivate에 대해 STAFF 금지 통합 테스트가 비어 있고, reservation cancel request DTO는 API Spec이 상위 문서/구현보다 덜 명확하다.
...
- Current behavior: API DTO는 `Instant`를 노출하고 테스트는 `"2026-05-10T10:00:00Z"` 같은 UTC `Z` 문자열을 기대한다. follow-up 처리 후 `ErrorResponse.timestamp`도 UTC `Z`로 생성한다.
- Expected behavior: persisted response timestamp와 `ErrorResponse.timestamp` 모두 ISO-8601 UTC `Z` 형식을 공식 API contract로 사용한다.
- Impact: API Spec, ErrorResponse 구현, API contract tests가 같은 UTC `Z` 형식으로 정렬됐다.
- Recommended action: 완료. production DTO의 `Instant` 타입은 유지하고 API Spec/Test Strategy/Notion 원본 문서를 UTC `Z` 기준으로 갱신했다.
- Follow-up resolution: `Decide API date-time response format`에서 MVP 구현 범위를 넓히지 않기 위해 persisted response timestamp와 `ErrorResponse.timestamp` 모두 ISO-8601 UTC `Z` 형식을 공식 API contract로 결정했다. API Spec 예시와 API test expectation은 UTC `Z` 기준으로 정렬한다.
...
### F-06: PassProduct optional fields are implemented correctly, and API Spec now documents optionality
...
- Expected behavior: Functional Spec marks both as optional, so implementation behavior is acceptable. API Spec now states that `price` and `validDays` are optional in create requests and nullable in responses.
- Impact: Resolved. External API readers no longer need to infer optionality from examples.
- Recommended action: Done in `Clarify PassProduct optional fields in API Spec`.
- Risk and effort estimate: 낮은 risk, 낮은 effort.
...
1. API date-time format decision 정리. `(Done: UTC Z contract로 결정)`
   ...
   - Acceptance: API Spec documents current UTC `Z` output and implementation/tests use the same format.
...
3. API Spec PassProduct optional fields 명확화. `(Done)`
