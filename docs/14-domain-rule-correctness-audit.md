...
감사 당시 문서에는 PRD/Functional Spec의 실제 MVP use case보다 넓은 상태 전이가 일부 남아 있었다. Member 재활성화는 `Clarify Member activation MVP scope`에서 MVP public use case/API가 아닌 future scope로 정리했다. MemberPass `expire()`/`cancel()`과 ClassSession `close()`는 여전히 PRD MVP scope와 현재 API/use case에 없으므로, 구현 누락으로 바로 판단하기보다 문서 드리프트로 분리하고 제품 결정 없이 production code를 넓히지 않는 것이 맞다.
...
- Expected behavior: PRD and Functional Spec feature sections list member registration/query/deactivation, not member reactivation. Domain Model and Test Strategy should not imply `INACTIVE -> ACTIVE` as MVP coverage.
- Impact: Resolved by clarifying Member reactivation as future product scope, not an MVP implementation obligation.
- Recommended action: Keep Member reactivation out of production/API work until a product decision updates PRD, Functional Spec, API Spec, and Test Strategy.
- Risk and effort estimate: Low production risk, low documentation effort.
- Resolution note: `Clarify Member activation MVP scope` keeps Member reactivation out of MVP public use cases/APIs and clarifies the Domain Model/Test Strategy expectations as future product scope.
...
1. Member activation scope 문서 정리.
   ...
   - Acceptance: Resolved by keeping Member reactivation out of MVP public use cases/APIs and removing lower-priority MVP docs/test expectations.
