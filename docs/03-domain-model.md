...
`activate()`는 MVP public use case/API가 아니다. 회원 재활성화가 필요해지면 별도 product decision으로 API와 권한 정책을 정의한 뒤 future/internal lifecycle transition으로 추가한다.
...
`expire()`와 `cancel()`은 MVP public use case/API가 아니다. `EXPIRED`와 `CANCELED` 상태는 예약 가능 여부와 복구 가능 여부를 판단하는 내부/future lifecycle state로 유지하되, batch expiration 또는 pass cancellation workflow를 MVP 구현 의무로 해석하지 않는다.
향후 public use case로 추가하려면 PRD/Functional Spec/API Spec/Test Strategy를 먼저 갱신한다.
...
`close()`는 MVP public use case/API가 아니다. `CLOSED` 상태는 예약 가능 여부를 판단하는 내부/future lifecycle state로 유지하되, class closing 또는 class update workflow를 MVP 구현 의무로 해석하지 않는다.
향후 public use case로 추가하려면 PRD/Functional Spec/API Spec/Test Strategy를 먼저 갱신한다.
...
| 현재 상태 | 이벤트/메서드 | 다음 상태 | 허용 여부 | 비고 |
| --- | --- | --- | --- | --- |
| ACTIVE | deactivate() | INACTIVE | MVP 허용 | 이후 신규 예약 불가 |
| INACTIVE | deactivate() | INACTIVE | 멱등 허용 가능 | 구현 정책으로 결정 |

`INACTIVE -> ACTIVE` 회원 재활성화는 현재 MVP 상태 전이 의무가 아니다. 향후 public use case로 추가하려면 PRD/Functional Spec/API Spec/Test Strategy를 먼저 갱신한다.
...
`ACTIVE/EXHAUSTED -> EXPIRED` 만료 전이와 `ACTIVE/EXHAUSTED -> CANCELED` 이용권 취소 전이는 future/internal lifecycle transition이다.
현재 MVP는 만료/취소 상태의 guard 동작만 정의하며, MemberPass `expire()`/`cancel()` use case, endpoint, batch job, test obligation은 포함하지 않는다.
...
`OPEN -> CLOSED` close 전이와 `CLOSED -> CANCELED` 전이는 future/internal lifecycle transition이다.
현재 MVP는 `CLOSED` 상태의 예약 차단 guard만 정의하며, ClassSession `close()` use case, endpoint, class update workflow, test obligation은 포함하지 않는다.
